...
func main() {
	...
	fmt.Printf("But b[len(b)] causes an out of bounds error")
	fmt.Println("======")
	c := make([]int, 3)
	fmt.Printf("c is : %T \n", c)
	...
	fmt.Printf("But d[5:] is out of bounds since d missing high index defaults to slice length (not array length): %d \n", len(d))
	_ = d[5:]
}
